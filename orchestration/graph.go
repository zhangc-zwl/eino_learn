...
{toolsInfo}
		...
		return schema.StreamReaderWithConvert(input, func(input []*schema.Message) (output *schema.Message, err error) {
			if len(input) > 0 {
				...
				for _, msg := range input {
					fmt.Println("tool name:", msg.Name)
					fmt.Println("tool role:", msg.Role)
					fmt.Println("tool result:", msg.Content)
				}
				fmt.Println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
				return input[0], nil
			}
	...
	// 🔍 调试：查看完整的输出对象
	println("\n=== 调试：输出对象信息 ===")
	println("output == nil:", output == nil)
	println("output.Role:", output.Role)
	println("output.Content 长度:", len(output.Content))
	println("output.ReasoningContent 长度:", len(output.ReasoningContent))
	if output.ToolCalls != nil {
		println("output.ToolCalls 数量:", len(output.ToolCalls))
	}
	println("===========================\n")
